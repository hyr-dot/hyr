...
class Rho_pollard:
    ...
    def search_j0(self,h0):
        '''
            PRIVATE FUNCTION
            Find the first value of j_0, ie when h_i = h'_i (h is a sequence defined by : for all i, h_i = f(h_(i-1). )
        '''
        print("[*] STEP 1 ...")
        h = self.f(h0)
    ...
    def search_i0(self,h0, h0_pprime):
        '''
           Find the first value of i_0, ie when h_i = h''_i. (h is a sequence defined by : for all i, h_i = f(h_(i-1).)
        '''
...
def hash_func(clear) :
    # MD5 hash
    return sm3.sm3_hash(func.bytes_to_list(clear.encode()))[0:12]
start=time.time()
pollard = Rho_pollard(hash_func, 1000, is_printing_hash = False)# Maximum length (in char) of the random word generated by Pollard's Rho algorithm
